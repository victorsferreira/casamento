...
<?php
$enable_content = get_theme_mod( 'catch_wedding_featured_content_option', 'disabled' );

if ( ! catch_wedding_check_section( $enable_content ) ) {
	// Bail if featured content is disabled.
	return;
}
	$featured_posts = catch_wedding_get_featured_posts();

	if ( empty( $featured_posts ) ) {
		return;
	}

	$title     = get_option( 'featured_content_title', esc_html__( 'Contents', 'catch-wedding' ) );
