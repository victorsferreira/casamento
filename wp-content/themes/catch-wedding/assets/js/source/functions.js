/* global catchWeddingScreenReaderText */
/**
...
( function( $ ) {
    ...
    $( function() {
        $( document ).ready( function() {
            $('.featured-content-wrapper .hentry-inner, .guest-book-content-wrapper .hentry-inner').matchHeight();
        });
    });
    ...
    function initMainNavigation( container ) {
        ...
        var dropdownToggle = $( '<button />', { 'class': 'dropdown-toggle', 'aria-expanded': false })
            .append( catchWeddingScreenReaderText.icon )
            .append( $( '<span />', { 'class': 'screen-reader-text', text: catchWeddingScreenReaderText.expand }) );
        ...
        container.find( '.current-menu-ancestor > button' )
            ...
            .text( catchWeddingScreenReaderText.collapse );
        ...
        container.find( '.dropdown-toggle' ).click( function( e ) {
            ...
            // jscs:enable
            screenReaderSpan.text( screenReaderSpan.text() === catchWeddingScreenReaderText.expand ? catchWeddingScreenReaderText.collapse : catchWeddingScreenReaderText.expand );
        } );
    }
    ...
    ( function() {
        ...
        menuToggle.on( 'click.catchWedding', function() {
    ...
    ( function() {
        ...
        function toggleFocusClassTouchScreen() {
            if ( window.innerWidth >= 1024 ) {
                $( document.body ).on( 'touchstart.catchWedding', function( e ) {
                    if ( ! $( e.target ).closest( '.main-navigation li' ).length ) {
                        $( '.main-navigation li' ).removeClass( 'focus' );
                    }
                } );
                siteNavigation.find( '.menu-item-has-children > a' ).on( 'touchstart.catchWedding', function( e ) {
                ...
                siteNavigation.find( '.menu-item-has-children > a' ).unbind( 'touchstart.catchWedding' );
            }
        }

        if ( 'ontouchstart' in window ) {
            $( window ).on( 'resize.catchWedding', toggleFocusClassTouchScreen );
            toggleFocusClassTouchScreen();
        }

        siteNavigation.find( 'a' ).on( 'focus.catchWedding blur.catchWedding', function() {
