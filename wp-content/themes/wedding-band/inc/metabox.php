...
function wedding_band_save_sidebar_layout( $post_id ) { 
    global $wedding_band_sidebar_layout, $post; 
    ...
    foreach( $wedding_band_sidebar_layout as $field ){  
        ...
        $old = get_post_meta( $post_id, 'wedding_band_sidebar_layout', true); 
        $new = sanitize_text_field( $_POST['wedding_band_sidebar_layout'] );
        if ( $new && $new != $old ) {  
            update_post_meta( $post_id, 'wedding_band_sidebar_layout', $new );  
        } elseif ( '' == $new && $old ) {  
            delete_post_meta( $post_id,'wedding_band_sidebar_layout', $old );  
        }  
    } // end foreach   
